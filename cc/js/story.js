...
get_char_table()
  ...
  .then((js) => {
    ...
    Object.values(storyReview).forEach((x) => {
      ...
      else if (x.entryType.startsWith("MINI_")) storyTypes.mini.push(x.id);
      else storyTypes.side.push(x.id);
    });

    storyTypes.record.sort((a, b) => {
      let na = (
        operatorData[charCodeMap[a.split("story_")[1].split("_")[0]]].name +
        " " +
        a.split("_").slice(-1)
      ).toLowerCase();
      let nb = (
        operatorData[charCodeMap[b.split("story_")[1].split("_")[0]]].name +
        " " +
        b.split("_").slice(-1)
      ).toLowerCase();
      if (na < nb) return -1;
        ...
        switch (uppercat) {
          ...
          case "side":
            if (d.storyCode) {
              ...
              name = d.storyCode;
              if (["Before", "After"].includes(pos)) name += " " + pos;
            }
            break;
          case "record":
    ...
    function buildSecondSelector(cat, trigger = true) {
      ...
      switch (cat) {
        ...
        case "record":
          namefunc = (n) => {
            let name =
              operatorData[charCodeMap[n.split("story_")[1].split("_")[0]]]
                .name;
    ...
    Array.from(document.getElementsByClassName("story_next")).forEach((e) => {
      e.onclick = () => {
        ...
        currentCategory.options[
          ++currentCategory.selectedIndex % currentCategory.options.length
        ].selected = true;
    ...
    function loadFromHash() {
      ...
      Array.from(document.getElementById("thirdCatSelect").options).forEach(
        (o) => {
        ...
        if (
          ...
          switch (cmd.toLowerCase()) {
            ...
            case "playsound":
              ...
              audio.setAttribute("data-defvol", args.volume || 0.8);
              ...
              if (cmd.toLowerCase() == "playsound") {
                ...
                function playSound() {
                  ...
                  if (this.audio.volume == 0) this.audio.volume = 0.5;
