...
fetch("./json/cctitles.json")
  ...
  .then((js) => {
    ...
    Object.keys(usedata).forEach((k) => {
      if (usedata[k].duplicate_of) {
        usedata[usedata[k].duplicate_of].squad = [
          ...new Set(
            usedata[usedata[k].duplicate_of].squad.concat(usedata[k].squad)
          ),
        ];
        delete usedata[k];
      }
    ...
    UPPER_BOUNDS =
      LOWER_BOUNDS +
      getStandardDeviation(Object.values(useCountMap).filter((x) => x != 0));
    ...
    function getBarLabels(data) {
      ...
      return data.map((x) =>
        x[0].name == "Skadi the Corrupting Heart" ? "Skadiva" : x[0].name
      );
    }
    ...
    let scatterImages = sortedData.map((x) => {
      ...
      i.src =
        "https://aceship.github.io/AN-EN-Tags/img/avatars/" + x[3] + ".png";
      i.opname = x[0].name;
      return i;
    });
    ...
    function redrawCharts(sortName) {
      ...
      scatterPlot.options.scales.y.ticks.callback = (value, index, values) =>
        value;
      switch (sortName) {
        case "Rarity":
        ...
        case "Class":
          scatterData = sortedData.map((d) => {
            return {
              x: d[1],
              y: Object.keys(classList).indexOf(classMap[d[0].name]),
              r: imgSize / 2,
            };
          });
          scatterPlot.options.scales.y.ticks.callback = (
    ...
    document.getElementById("viewType").onclick = () => {
      ...
      Array.from(
        document.getElementById("chartDiv").querySelectorAll(".chartOption")
